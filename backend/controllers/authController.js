...
/* ======================================================
   LOGIN (2FA + TRUSTED DEVICE + EXPIRY)
====================================================== */
...
/* ======================================================
   VERIFY 2FA OTP (ATTEMPT LIMIT + TRUST DEVICE)
====================================================== */
export const verify2FAOtp = async (req, res) => {
  try {
    ...
    if (attempts > 5) {
      return res
        .status(429)
        .json({ msg: "Too many invalid attempts" });
    }
    ...
    /* -----------------------------------------------
       🔐 TRUST DEVICE (WITH VERIFIED DATE)
    ------------------------------------------------ */
    if (rememberDevice) {
      const deviceId = getDeviceId(req);
...
export const resend2FA = async (req, res) => {
  try {
    ...
    if (count > 3) {
      return res
        .status(429)
        .json({ msg: "Too many OTP requests" });
    }
