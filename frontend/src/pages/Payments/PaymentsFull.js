...
export default function PaymentsPageFull(){
  ...
  async function payStripe(){
    const res = await fetch('/api/payments/stripe/create-payment-intent', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ amount }) });
  ...
  async function payMpesa(){
    const res = await fetch('/api/payments/mpesa/stk', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ amount, phone: '2547XXXXXXXX' }) });
